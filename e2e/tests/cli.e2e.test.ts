...
describe("CLI E2E Tests", () => {
   ...
   */
  async function runCliCommand(args: string[], options: { cwd?: string; expectError?: boolean } = {}): Promise<{
    stdout: string;
    stderr: string;
    exitCode: number;
  }> {
  ...
  describe("extract command", () => {
    it("should extract translation keys and generate POT file", async () => {
      const result = await runCliCommand([
        ...
        "--include", "**/*.messages.ts",
        "--source", fixturesDir,
        "--output", path.join(testDir, "extracted.pot")
      ]);
    ...
    it("should respect exclude patterns", async () => {
      ...
      fs.writeFileSync(path.join(fixturesDir, "node_modules", "test.messages.ts"), `
export const ExcludedMessages = {
  shouldNotBeIncluded: (): string => "This should be excluded",
      ...
      const result = await runCliCommand([
        ...
        "--include", "**/*.messages.ts",
        "--source", fixturesDir,
        "--output", path.join(testDir, "filtered.pot")
      ]);
  ...
  describe("sync command", () => {
    it("should sync PO files with template", async () => {
      const result = await runCliCommand([
        "sync",
        "--template", path.join(testDir, "locales", "messages.pot"),
        "--po-files", path.join(testDir, "locales", "*.po")
      ]);
    ...
    it("should use config defaults when no options specified", async () => {
      ...
      const extractResult = await runCliCommand([
        "extract",
        "--include", "**/*.messages.ts",
        "--source", fixturesDir
      ]);
  ...
  describe("convert command", () => {
    it("should convert single PO file to JSON", async () => {
      const result = await runCliCommand([
        "convert",
        "--input", path.join(testDir, "locales", "fr.po"),
        "--output", path.join(testDir, "fr.json")
      ]);
...
`;
      ...
      const result = await runCliCommand([
        "convert",
        "--input", path.join(testDir, "locales", "*.po"),
        "--output", outputDir,
        "--batch"
      ]);
    ...
    it("should support custom indentation", async () => {
      const result = await runCliCommand([
        "convert",
        "--input", path.join(testDir, "locales", "fr.po"),
        "--output", path.join(testDir, "fr-indented.json"),
        "--indent", "4"
      ]);
    ...
    it("should support top-level key wrapping", async () => {
      const result = await runCliCommand([
        "convert",
        "--input", path.join(testDir, "locales", "fr.po"),
        "--output", path.join(testDir, "fr-wrapped.json"),
  ...
  describe("status command", () => {
    it("should show translation status for directory", async () => {
      const result = await runCliCommand([
        "status",
        "--directory", path.join(testDir, "locales")
      ]);
    ...
    it("should handle non-existent directory gracefully", async () => {
      const result = await runCliCommand([
        "status",
        "--directory", path.join(testDir, "non-existent")
      ], { expectError: true });
