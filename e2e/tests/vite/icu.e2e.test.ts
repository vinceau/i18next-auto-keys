...
const ICU_TEST_CONFIGURATIONS = {
  ...
  icuIndexed: createViteConfig({
    ...
    resolveAlias: {
      "./messages/replay-browser.messages": path.resolve(
        __dirname,
        "../../fixtures/messages/replay-browser-indexed.messages.ts"
      ),
    },
  }),
};
...
describe("ICU Vite E2E Tests", () => {
  ...
  describe.each(Object.entries(ICU_TEST_CONFIGURATIONS))(
    "Configuration: %s",
    (configName: string, configWithPath: any) => {
      let buildResult: {
        bundlePath: string;
        translationsPath: string;
      };
